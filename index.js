...
inputbox1.addEventListener('input', e => {
    ...
    if (value.charAt(0) == '.') {
        value = `0${value}`
    } else if (value.charAt(0) == '0' && !value.includes('.') && value.length > 1) {
        value = value.slice(1)
    }

    let temp = value.slice(0, 15);
    inputbox1.value = temp.includes('.') ? temp.split('.')[0] + '.' + temp.split('.')[1].slice(0, 5) : temp
...
inputbox2.addEventListener('input', e => {
    ...
    let temp = value.slice(0, 15);
    inputbox2.value = temp.includes('.') ? temp.split('.')[0] + '.' + temp.split('.')[1].slice(0, 5) : temp
